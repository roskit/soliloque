...
//           					VERSION POUR OS X (2006)                                              *
//           				Last update: 5 Nov. 2013 for SuperCollider 3.6.5                                          *
// (c) 2002, 2005, 2006 by Fabien Levy, Thomas Seelig and Frederic Roskam. All rights reserved.        *
//******************************************************************************************************


  //******************************************************************************************************
  //*************          I DECLARATION DES VARIABLES GENERALES   ***************************************
  //******************************************************************************************************

  //--- globale Variables and Fonctions ---------------------------
  var	theSR,
  w,x,				// main window and wait window
  sampleView1,
  sampleView2,
  sampleView3,
  sampleView4,
  ...
  // ---------------- I.1 Variables diverses------------------------

  rect, sawDown, tri, sawUp, recti, sawDowni, sawUpi, fadeIn, envelopes, //instruments
  cosFunc, sinFunc, rampFunc, compFunc1, // fonctions cos, droites et complexes

  // ---------------- I.2 Soundfiles------------------------
  theSound1, theSound2, theSound3, theSound4, theSound5, theSound6,
  soundFile1, soundFile2, soundFile3, soundFile4, soundFile5, soundFile6,

  // ---------------- I.3 Variables from sound file analysis ------------------------
  ...
  channelNumberBox,						// Text box for the channels
 // s.options.numOutputBusChannels = 16; // CORRECTION SEPT 2008 ANDRE BATETSKI


  timeOffset = 5.0,						 //t0zeit4,
  globalT1 = 0,
  globalT2 = 0,
  globalT31 = 0,
  globalT32 = 0,
  ...
  // set the interpreter variable s to the server object.
  //Server.default = Server.internal; // can be replaced by Server.local;
  s = Server.default;
  //s.options.sampleRate = 48000; // ROSKAM 05/11/2013
  o = OSCresponderNode(s.addr, '/tr', { |time, resp, msg| msg.postln }).add;

  x = Window.new("SOLILOQUE", Rect.new(400, 300, 200, 100), false);
  x.front;

  StaticText( x, Rect.new(60 , 30 , 100, 20)).string = "Please wait...";
  //{ x.close; nil }.defer(5); // next application event cycle
...
teil4 = {
		  ...
		  durations = StaticText.new( w, Rect.new(510 , 456 , 400, 42));
		  durations.string = texttime;
...
// Nouvelle version malheureusement manque les petits glissandi qui rendaient la chose int�ressante
...
//--- Build new window -------------------
w = Window.new("SOLILOQUE SUR [X, X, X ET X]", Rect.new(150, 150, 1010, 525), false);
...
// Soliloque.app/Contents/Resources/sounds/
sampleView1 = SampleView.new(w, 0, 	0, "sample 1", "Soliloque.app/Contents/Resources/sounds/Sample1.wav");
sampleView2 = SampleView.new(w, 0, 	150, "sample 2", "Soliloque.app/Contents/Resources/sounds/Sample2.wav");
sampleView3 = SampleView.new(w, 0, 	300, "sample 3", "Soliloque.app/Contents/Resources/sounds/Sample3.wav");
sampleView4 = SampleView.new(w, 500, 	0, "sample 4", "Soliloque.app/Contents/Resources/sounds/Sample4.wav");
sampleView5 = SampleView.new(w, 500, 	150, "sample 5", "Soliloque.app/Contents/Resources/sounds/Sample5.wav");
sampleView6 = SampleView.new(w, 500, 	300, "sample 6", "Soliloque.app/Contents/Resources/sounds/Sample6.wav");

("SOLILOQUE: SampleView (library) version " ++ sampleView1.version).postln;



// Extra text on the main window
StaticText( w, Rect.new(12 , 447 , 177, 19)).string = "composition by Fabien Levy";
StaticText( w, Rect.new(12 , 465 , 410, 19)).string = "programmed on SC2 by Thomas Seelig (c) 2002";
StaticText( w, Rect.new(12 , 483 , 410, 19)).string = "transcribed on SC3 by Frederic Roskam (c) 2006&2013";
StaticText( w, Rect.new(12 , 501 , 410, 19)).string = "special thanks to Thomas Noll (canons of Vuza) ";

// Number of channels
StaticText( w, Rect.new(937, 490, 80, 19)).string = "channels";
channelNumberBox = NumberBox(w, Rect(912, 492, 20, 16));
...
// Go button
theGoButton = Button( w, Rect.new(810, 465, 90, 20), "soliloque go !");
theGoButton.states = [	["soliloque go!",Color.black,Color.new255(177,237,105)]];
theGoButton.action = {| view |
	"SOLILOQUE: Go".postln;

    //--- SoundFiles -------------------------
    soundFile1 = sampleView1.itsBuffer;
    soundFile2 = sampleView2.itsBuffer;
    soundFile3 = sampleView3.itsBuffer;
    soundFile4 = sampleView4.itsBuffer;
    soundFile5 = sampleView5.itsBuffer;
    soundFile6 = sampleView6.itsBuffer;

    //--- Steuerparameter -------------------
    tmax1	= sampleView1.tmax / theSR;
    tmx1	= sampleView1.tmx / theSR;
    tmin1	= sampleView1.tmin / theSR;
    fmax1	= sampleView1.fmax;
    fmx1	= sampleView1.fmx;
    fmin1	= sampleView1.fmin;
    vmax1	= sampleView1.vmax;
    vmx1	= sampleView1.vmx;
    vmin1	= sampleView1.vmin;
    l1	= sampleView1.len;

    tmax2	= sampleView2.tmax / theSR;
    tmx2	= sampleView2.tmx / theSR;
    tmin2	= sampleView2.tmin / theSR;
    fmax2	= sampleView2.fmax;
    fmx2	= sampleView2.fmx;
    fmin2	= sampleView2.fmin;
    vmax2	= sampleView2.vmax;
    vmx2	= sampleView2.vmx;
    vmin2	= sampleView2.vmin;
    l2	= sampleView2.len;

    tmax3	= sampleView3.tmax / theSR;
    tmx3	= sampleView3.tmx / theSR;
    tmin3	= sampleView3.tmin / theSR;
    fmax3	= sampleView3.fmax;
    fmx3	= sampleView3.fmx;
    fmin3	= sampleView3.fmin;
    vmax3	= sampleView3.vmax;
    vmx3	= sampleView3.vmx;
    vmin3	= sampleView3.vmin;
    l3	= sampleView3.len;

    tmax4	= sampleView4.tmax / theSR;
    tmx4	= sampleView4.tmx / theSR;
    tmin4	= sampleView4.tmin / theSR;
    fmax4	= sampleView4.fmax;
    fmx4	= sampleView4.fmx;
    fmin4	= sampleView4.fmin;
    vmax4	= sampleView4.vmax;
    vmx4	= sampleView4.vmx;
    vmin4	= sampleView4.vmin;
    l4	= sampleView4.len;

    tmax5	= sampleView5.tmax / theSR;
    tmx5	= sampleView5.tmx / theSR;
    tmin5	= sampleView5.tmin / theSR;
    fmax5	= sampleView5.fmax;
    fmx5	= sampleView5.fmx;
    fmin5	= sampleView5.fmin;
    vmax5	= sampleView5.vmax;
    vmx5	= sampleView5.vmx;
    vmin5	= sampleView5.vmin;
    l5	= sampleView5.len;

    tmax6	= sampleView6.tmax / theSR;
    tmx6	= sampleView6.tmx / theSR;
    tmin6	= sampleView6.tmin / theSR;
    fmax6	= sampleView6.fmax;
    fmx6	= sampleView6.fmx;
    fmin6	= sampleView6.fmin;
    vmax6	= sampleView6.vmax;
    vmx6	= sampleView6.vmx;
    vmin6	= sampleView6.vmin;
    l6	= sampleView6.len;
    ...
    "SOLILOQUE: Load SYNTHDEFs".postln;
    	SynthDef ("instFunc",
	{
		arg i_out = 0,
		...
		var   out,envelopes,env;  // Length in Samples

		// DEBUG
		//SendTrig.kr(Impulse.kr(1), 0, start);
	...
	//--- Bank of random frequency resonators ---------------------------------------------
	...
	//--- Bank of fixed frequency resonators ---------------------------------------------
	...
	SynthDef("instXFade", {
		...
		// DEBUG
		//SendTrig.kr(Impulse.kr(1), 0, theCrossFreq);
	...
	// Stop button
	theStopButton = Button( w, Rect.new(906, 465, 90, 20), "soliloque go !");
	theStopButton.states = [["STOP",Color.white,Color.new255(136,27,20)]];
	theStopButton.action = {| view |
     	"SOLILOQUE: Stop".postln;
    		~ptpar.stop;
    		Server.freeAll;
    		durations.string = ""; // ROSKAM: bug if no duration already calculated
	};
